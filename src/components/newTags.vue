      ...
      <div class="tags-list-wrapper" ref="list-container">
        <transition name="bounce" mode="in-out">
          <ul class="tags-list" ref="tags-container">
            <li class="tags-item" v-for="(item,index) in tags.value" :key="index">
              <div v-bind:class="currentPageRouter.fullPath==item.path?['select-item tags-item-button']:['tags-item-button']" @click="handelOnclik(item,index)">
                {{item.title}}
...
export default {
  ...
  setup(props, { refs, root }) {
    ...
    const currentPageRouter = useRoute() //当前路由对象
    //定义变量
    const tags = reactive([]);
    //定义函数
    ...
    const deleteTag = (item, $event) => {
      ...
      for (let i in tags) {
        if (tags[i]["path"] == item.path) {
          ...
          if (item.path == currentPageRouter.fullPath) {
            if (Number(i) == 0) {
    ...
    //设置标签
    const setTags = route => {
      const isExist = (ctx.tags.value).some(item => {
        return item["path"] == route.fullPath;
      });
      const isExistName = (ctx.tags.value).some(item => {
        return route.name ? item["name"] == route.name : false;
      });
      let ExistIndex = 0;
      ExistIndex = (ctx.tags.value).map(i => i.name).indexOf(route.name);
      console.log(ExistIndex, "ExistIndex", route.meta)
      if (!isExist) {
        var tags = ctx.tags.value;
        if (isExistName) {
          //目前的tags已存在点击的这个路由
    ...
    const closeOthers = () => {
      var route = currentPageRouter;
      var homeTag = (ctx.tags.value).filter(item => {
    ...
    onBeforeMount(() => {
      ...
      ctx.setTags(currentPageRouter);
    });

    //计算属性
    //监听数据
    // watch(() => router.path, () => {
    //   console.log('监听到变化', router)
    //   // ctx.setTags(currentPageRouter);
    // })
