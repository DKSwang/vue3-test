 ...
 * @LastEditTime: 2021-01-29 08:39:35
 * @Description:
 */
import {
    createApp
...
app.directive("dictType", {
    bind(el, binding, vnode, oldVnode) {
        var dictTypeArr = binding.value;
        for (var i = 0, len = dictTypeArr.length; i < len; i++) {
            //vuex中取不到该字典项，则通过服务器请求获取
            if (Store.state["common"][dictTypeArr[i]].length == 0) {
                getDictByType({
                    ...
                    if (res["code"] == 200) {
                        for (let j in res.data) {
                            //获取到字典数据，存储到vuex中
                            Store.state["common"][j] = res.data[j];
                        }
                    }
                });
                break;
